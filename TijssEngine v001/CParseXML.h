...
public:
	...
	bool ParseFile( const string& fileName, int bufferSize = 32768 );
...
protected:
	...
	// Returns defaultValue if the name isn't in the list
	static int GetAttributeInt( SAttribute* attrs, const string& name,
	                               int defaultValue = 0 );
	
	// Return the float value associated with the given name in the given attribute list
	// Returns defaultValue if the name isn't in the list
	static float GetAttributeFloat( SAttribute* attrs, const string& name,

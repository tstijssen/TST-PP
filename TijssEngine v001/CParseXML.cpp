...
bool CParseXML::ParseFile( const string& fileName, int bufferSize /*= 8192*/ )
{
	...
	while (!feof( file ))
	{
		...
		int charsRead = static_cast<int>(fread( buffer.get(), 1, bufferSize, file ));
...
// Returns defaultValue if the name isn't in the list
int CParseXML::GetAttributeInt( SAttribute* attrs, const string& name,
                                   int defaultValue /*= 0*/ )
{
	for (int i = 0; attrs[i].name != 0; ++i)
	{
		if (attrs[i].name == name)
		{
			return static_cast<int>(atoi( attrs[i].value ));
		}
	}
	return defaultValue;
}
...
// Returns defaultValue if the name isn't in the list
float CParseXML::GetAttributeFloat( SAttribute* attrs, const string& name,
                                       float defaultValue /*= 0.0f*/ )
{
	for (int i = 0; attrs[i].name != 0; ++i)
	{
		if (attrs[i].name == name)
		{
			return static_cast<float>(atof( attrs[i].value ));
		}
	}
	return defaultValue;
}
